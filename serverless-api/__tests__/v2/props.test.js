...
describe('PROPS TESTS', () => {
  ...
  describe('POST /api/v2/props', () => {
    ...
    test('Successful prop creation', async () => {
      ...
      const response = await request(apiUrl)
        ...
        .send(prop)
      ;
      if (response.statusCode === 200) propId = response.body.prop.info.id;

      expect(response.body.error).toBeUndefined();
      expect(response.statusCode).toBe(200);
      expect(response.body.prop).toBeDefined();
      expect(response.body.prop.info.name).toBe(prop.name);
    });
  ...
  describe('DELETE /api/v2/props/:id', () => {
    let propId = null;
  ...
  describe('GET /api/v2/props/list', () => {
    test('List all props', async () => {
      const response = await request(apiUrl)
        .get('/api/v2/props/list')
        .set('Authorization', token)
      ;
      expect(response.body.error).toBeUndefined();
      expect(response.statusCode).toBe(200);

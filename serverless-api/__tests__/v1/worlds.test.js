...
describe('WORLDS TESTS', () => {
  ...
  describe('POST /api/v1/worlds', () => {
    ...
    test('Successful world creation', async () => {
      ...
      const response = await request(apiUrl)
        ...
        .send(world)
      ;
      if (response.statusCode === 200) worldId = response.body.world.id;

      expect(response.body.error).toBeUndefined();
      expect(response.statusCode).toBe(200);
      expect(response.body.world).toBeDefined();
      expect(response.body.world.name).toBe(world.name);
    });
  ...
  describe('DELETE /api/v1/worlds/:id', () => {
    ...
    test('World delete fail with non-existing id', async () => {
      const response = await request(apiUrl)
        .delete(`/api/v1/worlds/999999999`)
        .set('Authorization', token)
      ;
      expect(response.statusCode).toBe(404);
      expect(response.body.error.message).toBe('RecordNotExistException');
  ...
  describe('GET /api/v1/worlds/list', () => {

...
describe('EVENTS TESTS', () => {
  ...
  describe('POST /api/v1/events', () => {
    ...
    test('Successful event creation', async () => {
      ...
      const response = await request(apiUrl)
        ...
        .send(event)
      ;
      if (response.statusCode === 200) eventId = response.body.event.id;

      expect(response.body.error).toBeUndefined();
      expect(response.statusCode).toBe(200);
      expect(response.body.event).toBeDefined();
      expect(response.body.event.name).toBe(event.name);
    });
  ...
  describe('DELETE /api/v1/events/:id', () => {
    ...
    test('Event delete fail with non-existing id', async () => {
      const response = await request(apiUrl)
        .delete(`/api/v1/events/999999999`)
        .set('Authorization', token)
      ;
      expect(response.statusCode).toBe(404);
      expect(response.body.error.message).toBe('RecordNotExistException');
    ...
    test('Event deletion failed because it has world', async () => {
      ...
      const responseDeleteWorld = await shared.deleteWorld(responseWorld.body.world.id, apiUrl, token);
    ...
    test('Event deletion failed because it has guest', async () => {
      ...
      const responseDeleteGuest = await shared.deleteGuest(responseGuest.body.guest.id, apiUrl, token);
  ...
  describe('GET /api/v1/events/subscribed/:userId', () => {
    test('Get all subscribed events by userId', async () => {
      const userId = shared.userId;
      const response = await request(apiUrl)
        .get(`/api/v1/events/subscribed/${userId}`)
        .set('Authorization', token)
      ;
      expect(response.body.error).toBeUndefined();
      expect(response.statusCode).toBe(200);
  ...
  describe('GET /api/v1/events/user-in-event/:eventId', () => {
    let eventId = '';
    ...
    test('Check if user is in event', async () => {
      ...
      const userId = responseUser.body.user.id;
      ...
      const guestId = responseGuest.body.guest.id;

      /////

      const response = await request(apiUrl)
        .get(`/api/v1/events/user-in-event/${eventId}`)
        .set('Authorization', token)
      ;
      expect(response.body.error).toBeUndefined();
      expect(response.statusCode).toBe(200);

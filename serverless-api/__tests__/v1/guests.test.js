...
describe('GUESTS TESTS', () => {
  ...
  describe('POST /api/v1/guests', () => {
    ...
    test('Successful guest creation', async () => {
      ...
      const response = await request(apiUrl)
        ...
        .send(guest)
      ;
      if (response.statusCode === 200) guestId = response.body.guest.id;

      expect(response.body.error).toBeUndefined();
      expect(response.statusCode).toBe(200);
      expect(response.body.guest).toBeDefined();
      expect(response.body.guest.name).toBe(guest.name);
    });
  ...
  describe('DELETE /api/v1/guests/:id', () => {
    ...
    test('Guest delete fail with non-existing id', async () => {
      const response = await request(apiUrl)
        .delete(`/api/v1/guests/999999999`)
        .set('Authorization', token)
      ;
      expect(response.statusCode).toBe(404);
      expect(response.body.error.message).toBe('RecordNotExistException');
  ...
  describe('GET /api/v1/guests/list', () => {
